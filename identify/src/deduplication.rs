...
use crate::mimetype::identify_mimetype;
...
///
pub async fn dedupe_checksum<'a, R>(mut content: R) -> anyhow::Result<Deduplication>
where R: AsyncRead + AsyncSeek + Send + Unpin + 'a
{
    let mimetype = identify_mimetype(&mut content).await?;
    content.rewind().await?;

    dedupe_checksum_with_mimetype(content, mimetype).await
}
...
/// A [`Deduplication`] struct containing the checksum and the mimetype of the file.
///
pub async fn dedupe_checksum_with_mimetype<'a, R>(content: R, mimetype: impl Into<String>) -> anyhow::Result<Deduplication>
where R: AsyncRead + Send + Unpin + 'a
{
    ...
    Ok(Deduplication { checksum, mimetype })
}
...
mod tests {
    ...
    use crate::deduplication::dedupe_checksum_with_mimetype;

    #[tokio::test]
    async fn test_dedupe_checksum_message_no_data() {
        let content = Cursor::new(b"".to_vec());

        let dedupe = dedupe_checksum_with_mimetype(content, "message/rfc822").await.unwrap();

        assert_eq!(dedupe.checksum, "d41d8cd98f00b204e9800998ecf8427e");
    }
...
Phillip";
        ...
        assert_eq!(dedupe.checksum, "48746efe196a27e395f613b9c0773b8b");
    }
    ...
    async fn test_dedupe_checksum_md5_no_data() {
        ...
        let dedupe = dedupe_checksum_with_mimetype(content, "application/octet-stream").await.unwrap();
    ...
    async fn test_dedupe_checksum_md5() {
        ...
        assert_eq!(dedupe.checksum, "bccf69bd7101c797b298c8b5329b965f");
    }
}
