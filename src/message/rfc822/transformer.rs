...
impl MessageTransformer {
    ...
    pub fn transform<W>(&self, message: &Message, writer: &mut W) -> io::Result<()>
    where
        W: Write,
    {
    ...
    fn transform_header(&self, message: &Message, header_name: &str) -> Option<String> {
        ...
        match header_value {
            HeaderValue::Address(addr) => self.visitor.on_header_address(header_name, addr),

            HeaderValue::AddressList(addr_list) => {
                self.visitor.on_header_address_list(header_name, addr_list)
            }

            HeaderValue::Group(group) => self.visitor.on_header_group(header_name, group),

            HeaderValue::GroupList(group_list) => {
                self.visitor.on_header_group_list(header_name, group_list)
            }

            HeaderValue::Text(text) => self.visitor.on_header_text(header_name, text),
    ...
    fn write_if_some<W>(&self, writer: &mut W, value: Option<String>) -> io::Result<()>
    where
        W: Write,
    {
